...
```bash
npx skills add Heelc/kx-skills --skill=domain-modeling
```
...
[Source](https://github.com/Heelc/kx-skills/tree/main/skills/engineering/domain-modeling)
...
在 Codex 中可输入 `$` 并选择 `kx-skills:domain-modeling`，在 Claude Code 中可输入 `/domain-modeling`；任务匹配时，模型也可以隐式触发该 skill。
