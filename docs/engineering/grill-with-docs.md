...
```bash
npx skills add Heelc/kx-skills --skill=grill-with-docs
```
...
[Source](https://github.com/Heelc/kx-skills/tree/main/skills/engineering/grill-with-docs)
...
在 Codex 中输入 `$` 并选择 `kx-skills:grill-with-docs`；在 Claude Code 中输入 `/grill-with-docs`。该 skill 只能由用户显式启动。
