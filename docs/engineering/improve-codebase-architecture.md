...
```bash
npx skills add Heelc/kx-skills --skill=improve-codebase-architecture
```
...
[Source](https://github.com/Heelc/kx-skills/tree/main/skills/engineering/improve-codebase-architecture)

## What it does

`improve-codebase-architecture` scans a codebase for **deepening opportunities** — places where a shallow module (an interface nearly as complex as the thing it hides) could become a deep one — presents at most three credible candidates as a compact visual HTML report, then grills through whichever one you pick.
...
在 Codex 中输入 `$` 并选择 `kx-skills:improve-codebase-architecture`；在 Claude Code 中输入 `/improve-codebase-architecture`。该 skill 只能由用户显式启动。
...
The output is a browser-ready HTML file written to your OS temp directory — nothing lands in the repo. It stays under 100 KiB, uses CDN imports, and never embeds source or dependency dumps. Each candidate is a card with the files involved, the friction, a plain-English solution, the benefit in terms of locality and leverage, a before/after diagram, and a `Strong` / `Worth exploring` / `Speculative` badge. It closes with the one it would tackle first.
