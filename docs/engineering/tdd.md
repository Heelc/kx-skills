...
```bash
npx skills add Heelc/kx-skills --skill=tdd
```
...
[Source](https://github.com/Heelc/kx-skills/tree/main/skills/engineering/tdd)
...
在 Codex 中可输入 `$` 并选择 `kx-skills:tdd`，在 Claude Code 中可输入 `/tdd`；任务匹配时，模型也可以隐式触发该 skill。
