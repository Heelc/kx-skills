...
A skill whose invocation policy lets the agent see its **description** and fire it autonomously — and the human can still select its name, so model-invocation always _includes_ user reach. It pays **context load** in exchange for that discoverability and can be reached by other skills. In shared source, Claude Code expresses this by omitting `disable-model-invocation`; Codex expresses it by omitting `policy.allow_implicit_invocation: false` from `agents/openai.yaml`. Pick model-invocation only when the agent or another skill must reach it on its own.

_Avoid_: ability, tool, capability

### User-Invoked

A skill visible in the human's selector but excluded from implicit model invocation (user-_only_, where **model-invoked** is user-_and-agent_). It keeps the required **description** as selector metadata while its policy keeps that trigger out of implicit reach. Claude Code sets `disable-model-invocation: true`; Codex sets `policy.allow_implicit_invocation: false` in `agents/openai.yaml`. Another skill may recommend it but cannot fire it.

_Avoid_: procedure, workflow, command

### Description

The skill's required machine-readable summary. For a **model-invoked** skill it is also the top-level trigger and **context pointer** exposed to the model. For a **user-invoked** skill it remains human-facing selector metadata while platform policy blocks implicit invocation. Presence alone does not decide the invocation axis; Claude frontmatter and Codex `agents/openai.yaml` policy do.
...
The cost a **model-invoked** skill imposes on the agent's context window — its trigger metadata spends both tokens and attention. **User-invoked** skills escape that implicit trigger load because platform policy excludes them, even though their selector metadata still contains a description. This is the brake on splitting into more model-invoked skills.
...
A **user-invoked** skill whose job is to point at your other user-invoked skills — naming each and when to reach for it — so the human has one skill to remember instead of many. It can only hint, never fire them: invocation policy reserves those skills for the human. The cure for **cognitive load** when user-invoked skills multiply.
